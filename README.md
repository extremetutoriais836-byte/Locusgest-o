...
| Preços e desconto | `var PRECOS={consultoria:150, implementacao:500, …, descontoTrimestral:50}` |
...
A conta gira em torno de **operações gerenciadas**, não de lojas:

```
operações = nº de lojas × (plataforma === "Ambas" ? 2 : 1)
```

Cada plataforma é um painel separado, com cardápio, campanhas e ranqueamento próprios — por isso, escolher "Ambas" dobra a implementação e a gestão semanal, e o formulário passa a pedir o nome da loja em cada plataforma (os nomes costumam divergir entre iFood e 99Food).

Uma loja nas duas plataformas custa o mesmo que duas lojas em uma só. Se o modelo comercial mudar, o ponto único a ajustar é a função `unidades()`.
